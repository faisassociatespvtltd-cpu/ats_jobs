    ...
    <form method="POST" action="{{ route('job-postings.update', $jobPosting) }}" style="max-width: 800px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px; margin-bottom: 16px;">
            ...
            <div class="form-group">
                ...
                <select name="experience_level" class="form-control">
                    <option value="">Select...</option>
                    <option value="Fresher / Entry Level" {{ old('experience_level', $jobPosting->experience_level) == 'Fresher / Entry Level' ? 'selected' : '' }}>Fresher / Entry Level</option>
                    <option value="Junior (1-2 Years)" {{ old('experience_level', $jobPosting->experience_level) == 'Junior (1-2 Years)' ? 'selected' : '' }}>Junior (1-2 Years)</option>
                    <option value="Mid Level (3-5 Years)" {{ old('experience_level', $jobPosting->experience_level) == 'Mid Level (3-5 Years)' ? 'selected' : '' }}>Mid Level (3-5 Years)</option>
                    <option value="Senior (5+ Years)" {{ old('experience_level', $jobPosting->experience_level) == 'Senior (5+ Years)' ? 'selected' : '' }}>Senior (5+ Years)</option>
                    <option value="Expert" {{ old('experience_level', $jobPosting->experience_level) == 'Expert' ? 'selected' : '' }}>Expert</option>
                </select>
            </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
            <div class="form-group">
                ...
                <input type="text" name="education_required" class="form-control" value="{{ old('education_required', $jobPosting->education_required) }}" placeholder="e.g., Bachelor's in CS">
                @error('education_required') <div style="color: #d13438; font-size: 12px; margin-top: 4px;">{{ $message }}</div> @enderror
            </div>

            <div class="form-group">
                <label>Experience Required</label>
                <input type="text" name="experience_required" class="form-control" value="{{ old('experience_required', $jobPosting->experience_required) }}" placeholder="e.g., 3-5 years">
